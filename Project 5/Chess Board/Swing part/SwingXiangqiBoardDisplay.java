...
/** Rules for how we want a board to display for a game of Xiang Qi chess
 *
 * @author Harold Connamacher
 * @author Bruce Dong
 */
public class SwingXiangqiBoardDisplay implements SwingChessBoardDisplay{
  ...
  public void displayEmptySquare(JButton button, int row, int column) {
    ...
    button.setBackground(!((column >2 && column < 6) &&((row >=0 && row<3)|| (row >6 && row<11) ))
            ? new Color(238, 238, 210) : new Color(202, 194, 194));
    button.setText(null);
    button.setIcon(null);
  }
